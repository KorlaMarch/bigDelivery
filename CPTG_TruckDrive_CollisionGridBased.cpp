#include "CPTG_TruckDrive_CollisionGridBased.h"
 ...
 */
CPTG_TruckDrive_CollisionGridBased::CPTG_TruckDrive_CollisionGridBased() :
	V_MAX(.0), W_MAX(.0),
...
void CPTG_TruckDrive_CollisionGridBased::loadDefaultParams()
{
...
void CPTG_TruckDrive_CollisionGridBased::loadFromConfigFile(const mrpt::utils::CConfigFileBase &cfg,const std::string &sSection)
{
...
void CPTG_TruckDrive_CollisionGridBased::saveToConfigFile(mrpt::utils::CConfigFileBase &cfg,const std::string &sSection) const
{
	MRPT_START
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CPTG_TruckDrive_CollisionGridBased::simulateTrajectories(
		float			max_time,
		float			max_dist,
		unsigned int	max_n,
		float			diferencial_t,
		...
		float			*out_max_acc_w)
{
	...
	try
	{
		for (unsigned int k=0;k<m_alphaValuesCount;k++)
		{
			...
			while ( t < max_time && dist < max_dist && points.size() < max_n && fabs(girado) < 1.95 * M_PI )
			{
				...
				ptgTruckDriveSteeringFunction( alpha,t, x, y, phi, v,w );
	...
	{
		std::cout << format("[CPTG_TruckDrive_CollisionGridBased::simulateTrajectories] Simulation aborted: unexpected exception!\n");
	}

}

/** In this class, `out_action_cmd` contains: [0]: linear velocity (m/s),  [1]: angular velocity (rad/s) */
mrpt::kinematics::CVehicleVelCmdPtr CPTG_TruckDrive_CollisionGridBased::directionToMotionCommand(uint16_t k) const
{
	float v,w;
...
/*---------------------------------------------------------------
					getTPObstacle
  ---------------------------------------------------------------*/
const CPTG_TruckDrive_CollisionGridBased::TCollisionCell & CPTG_TruckDrive_CollisionGridBased::CCollisionGrid::getTPObstacle(
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CPTG_TruckDrive_CollisionGridBased::CCollisionGrid::updateCellInfo(
...
/*---------------------------------------------------------------
					Save to file
  ---------------------------------------------------------------*/
bool CPTG_TruckDrive_CollisionGridBased::saveColGridsToFile( const std::string &filename, const mrpt::math::CPolygon & computed_robotShape ) const
{
	try
	{
...
/*---------------------------------------------------------------
					Load from file
  ---------------------------------------------------------------*/
bool CPTG_TruckDrive_CollisionGridBased::loadColGridsFromFile( const std::string &filename, const mrpt::math::CPolygon & current_robotShape  )
{
	try
	{
...
/*---------------------------------------------------------------
					Save to file
  ---------------------------------------------------------------*/
bool CPTG_TruckDrive_CollisionGridBased::CCollisionGrid::saveToFile( mrpt::utils::CStream *f, const mrpt::math::CPolygon & computed_robotShape ) const
{
	try
	{
		if (!f) return false;
...
/*---------------------------------------------------------------
						loadFromFile
  ---------------------------------------------------------------*/
bool CPTG_TruckDrive_CollisionGridBased::CCollisionGrid::loadFromFile( mrpt::utils::CStream *f, const mrpt::math::CPolygon & current_robotShape  )
{
	try
	{
		if (!f) return false;
...
bool CPTG_TruckDrive_CollisionGridBased::inverseMap_WS2TP(double x, double y, int &out_k, double &out_d, double tolerance_dist) const
{
	using mrpt::math::square;
...
void CPTG_TruckDrive_CollisionGridBased::setRefDistance(const double refDist)
{
...
void CPTG_TruckDrive_CollisionGridBased::internal_processNewRobotShape()
{
	ASSERTMSG_(m_trajectory.empty(), "Changing robot shape not allowed in this class after initialization!");
}

void CPTG_TruckDrive_CollisionGridBased::internal_deinitialize()
{
	m_trajectory.clear(); // Free trajectories
}

void CPTG_TruckDrive_CollisionGridBased::internal_initialize(const std::string & cacheFilename, const bool verbose)
{
	using namespace std;
	...
	if (verbose)
		cout << endl << "[CPTG_TruckDrive_CollisionGridBased::initialize] Starting... *** THIS MAY TAKE A WHILE, BUT MUST BE COMPUTED ONLY ONCE!! **" << endl;
...
size_t CPTG_TruckDrive_CollisionGridBased::getPathStepCount(uint16_t k) const
{
...
void CPTG_TruckDrive_CollisionGridBased::getPathPose(uint16_t k, uint32_t step, mrpt::math::TPose2D &p) const
{
	ASSERT_(k<m_trajectory.size());
...
double CPTG_TruckDrive_CollisionGridBased::getPathDist(uint16_t k, uint32_t step) const
{
	ASSERT_(k<m_trajectory.size());
	ASSERT_(step<m_trajectory[k].size());
...
bool CPTG_TruckDrive_CollisionGridBased::getPathStepForDist(uint16_t k, double dist, uint32_t &out_step) const
{
	ASSERT_(k<m_trajectory.size());
...
void CPTG_TruckDrive_CollisionGridBased::updateTPObstacle(
	double ox, double oy,
...
void CPTG_TruckDrive_CollisionGridBased::updateTPObstacleSingle(double ox, double oy, uint16_t k, double &tp_obstacle_k) const
{
	ASSERTMSG_(!m_trajectory.empty(), "PTG has not been initialized!");
	const TCollisionCell & cell = m_collisionGrid.getTPObstacle(ox, oy);
	// Keep the minimum distance:
...
void CPTG_TruckDrive_CollisionGridBased::internal_readFromStream(mrpt::utils::CStream &in)
{
...
void CPTG_TruckDrive_CollisionGridBased::internal_writeToStream(mrpt::utils::CStream &out) const
{
...
mrpt::kinematics::CVehicleVelCmdPtr CPTG_TruckDrive_CollisionGridBased::getSupportedKinematicVelocityCommand() const
{
	return mrpt::kinematics::CVehicleVelCmdPtr( new mrpt::kinematics::CVehicleVelCmd_TruckDrive() );
}

double CPTG_TruckDrive_CollisionGridBased::getPathStepDuration() const
{
