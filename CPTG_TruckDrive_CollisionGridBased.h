...
namespace nav
{
	...
	/** Trajectory points in C-Space for non-holonomic robots \sa CPTG_TruckDrive_CollisionGridBased */
	...
	/** Base class for all PTGs suitable to non-holonomic, differentially-driven (or Ackermann) vehicles
	  ...
	  */
	class NAV_IMPEXP CPTG_TruckDrive_CollisionGridBased : public CPTG_RobotShape_Polygonal
	{
	public:
		/** The main method to be implemented in derived classes: it defines the differential-driven differential equation */
		virtual void ptgTruckDriveSteeringFunction( float alpha, float t, float x, float y, float phi, float &v, float &w) const = 0;
...
protected:
		CPTG_TruckDrive_CollisionGridBased();
		...
		{
		private:
			CPTG_TruckDrive_CollisionGridBased const * m_parent;

		public:
			CCollisionGrid(float x_min, float x_max,float y_min, float y_max, float resolution, CPTG_TruckDrive_CollisionGridBased* parent )
